...
public class ClassificationKnowledgeTool {
    ...
    @McpTool(
            ...
            description = """
                    ...
                    支出先や購入内容と利用目的の組み合わせは、次回の用途確認に利用できる情報として扱ってください。
                    ナレッジだけでは経費科目を確定できないことを、登録確認を省略する理由にしないでください。
                    登録条件を満たす場合は、経費登録の有無にかかわらず、処理を終える前にナレッジの登録内容を提示して確認してください。
                    確認文は「経費科目判定用ナレッジとして登録してよいですか？」のように、ナレッジ登録であることを明示してください。
                    経費登録への同意や、「登録して」という対象が曖昧な返答は、ナレッジ登録への同意として扱わないでください。
                    ...
                    register_expense と同じ応答で呼び出さないでください。
                    """,
            generateOutputSchema = false)
